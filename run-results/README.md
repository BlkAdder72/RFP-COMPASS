...
| B-python | `make_packet.py build` on the full PDF, rebuilt with the final tool. | 941 statements (the first version of the tool gave 1,188). All 941 page anchors match where their text starts. Sentences wrapped across lines and pages are rejoined; for example, the deadline "…by 3:00 P.M. (CST), on TUESDAY, MARCH 31, 2026." is one statement. Four running headers/footers were kept once and 159 repeats were set aside and reported. `verify`: PASS, full coverage. |
